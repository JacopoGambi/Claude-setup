...
hooks:
  pre: |
    ...
    FAILED_TESTS=$(npx @claude-flow/cli@latest memory search --query "$TASK failures" --limit 5 --failures-only --use-hnsw)
    if [ -n "$FAILED_TESTS" ]; then
      echo "⚠️  Learning from past test failures (HNSW-indexed)"
      npx @claude-flow/cli@latest hooks intelligence --action pattern-search --query "$TASK" --failures-only
    fi

    # 2. Find similar successful test patterns
    SUCCESSFUL_TESTS=$(npx @claude-flow/cli@latest memory search --query "$TASK" --limit 3 --min-score 0.9 --use-hnsw)
  ...
  post: |
    ...
    npx @claude-flow/cli@latest hooks post-task --task-id "tester-$(date +%s)" --success "$SUCCESS"
