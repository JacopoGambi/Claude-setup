...
hooks:
  pre: |
    ...
    SIMILAR_RESEARCH=$(npx @claude-flow/cli@latest memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    npx @claude-flow/cli@latest memory store --key "research_context_$(date +%s)" --value "$TASK"
  ...
  post: |
    ...
    npx @claude-flow/cli@latest memory search --query "research" --limit 5

    # 1. Calculate research quality metrics
    FINDINGS_COUNT=$(npx @claude-flow/cli@latest memory search --query "research" --count-only || echo "0")
    ...
    npx @claude-flow/cli@latest hooks post-task --task-id "researcher-$(date +%s)" --success "$SUCCESS"
