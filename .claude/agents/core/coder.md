...
hooks:
  pre: |
    ...
    SIMILAR_PATTERNS=$(npx @claude-flow/cli@latest memory search --query "$TASK" --limit 5 --min-score 0.8 --use-hnsw)
    ...
    FAILURES=$(npx @claude-flow/cli@latest memory search --query "$TASK failures" --limit 3 --failures-only)
    if [ -n "$FAILURES" ]; then
  ...
  post: |
    ...
    npx @claude-flow/cli@latest hooks post-task --task-id "coder-$(date +%s)" --success "$SUCCESS"
