...
hooks:
  pre_task: |
    ...
    npx @claude-flow/cli@latest hook pre-task --mode release-swarm --init-swarm
  post_edit: |
    echo "🔄 Synchronizing release swarm state and validating changes..."
    npx @claude-flow/cli@latest hook post-edit --mode release-swarm --sync-swarm
  post_task: |
    echo "🎯 Release swarm task completed. Coordinating final deployment..."
    npx @claude-flow/cli@latest hook post-task --mode release-swarm --finalize-release
  notification: |
    echo "📡 Broadcasting release completion across all swarm agents..."
    npx @claude-flow/cli@latest hook notification --mode release-swarm --broadcast
---
...
npx @claude-flow/cli@latest github release-plan \
  --commits "$COMMITS" \
...
npx @claude-flow/cli@latest github release-version \
  --strategy "semantic" \
  --analyze-changes \
  --check-breaking \
  --update-files
...
CHANGELOG=$(gh api repos/:owner/:repo/compare/${LAST_TAG}...HEAD \
  ...
  npx @claude-flow/cli@latest github generate-changelog)
...
npx @claude-flow/cli@latest github hotfix \
  --issue 789 \
    ...
    steps:
      ...
      - name: Initialize Release Swarm
        run: |
          ...
          npx @claude-flow/cli@latest github release-init \
            --tag $RELEASE_TAG \
      ...
      - name: Generate Release Assets
        run: |
          ...
          CHANGELOG=$(npx @claude-flow/cli@latest github release-changelog \
            --format markdown)
          ...
          npx @claude-flow/cli@latest github release-assets \
            --changelog \
            --binaries \
            --documentation
      ...
      - name: Publish Release
        run: |
          ...
          npx @claude-flow/cli@latest github release-publish \
            --platforms all
...
npx @claude-flow/cli@latest github cd-pipeline \
...
npx @claude-flow/cli@latest github release-docs \
  --api-changes \
  --migration-guide \
  --example-updates \
...
```bash
# NPM package release
npx @claude-flow/cli@latest github npm-release \
  --version patch \
  --test-all \
  --publish-beta \
...
npx @claude-flow/cli@latest github docker-release \
...
npx @claude-flow/cli@latest github mobile-release \
