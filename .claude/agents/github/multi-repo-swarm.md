...
npx @claude-flow/cli@latest github multi-repo-init \
...
npx @claude-flow/cli@latest github discover-repos \
  --repos "$REPOS" \
  --dependencies "$DEPS" \
...
echo "$MATCHING_REPOS" | while read -r repo; do
  ...
  # Execute task
  cd /tmp/$repo
  npx @claude-flow/cli@latest github task-execute \
...
npx @claude-flow/cli@latest github link-prs --urls "$PR_URLS"
```
...
npx @claude-flow/cli@latest github multi-repo-refactor \
...
npx @claude-flow/cli@latest github multi-repo-security \
  --scan-all \
...
npx @claude-flow/cli@latest github multi-repo-queue \
  --backend redis \
  --workers 10 \
  --priority-routing \
  --dead-letter-queue
...
npx @claude-flow/cli@latest github multi-repo-test \
  --setup-test-env \
  --link-services \
  --run-e2e \
  --tear-down
...
npx @claude-flow/cli@latest github to-monorepo \
  --analyze-repos \
  --suggest-structure \
  --preserve-history \
...
npx @claude-flow/cli@latest github multi-repo-dashboard \
  --port 3000 \
...
npx @claude-flow/cli@latest github dep-graph \
  --format mermaid \
  --include-agents \
  --show-data-flow
```
...
npx @claude-flow/cli@latest github health-check \
  --repos "org/*" \
...
npx @claude-flow/cli@latest github microservices \
...
npx @claude-flow/cli@latest github lib-update \
...
npx @claude-flow/cli@latest github org-policy \
...
npx @claude-flow/cli@latest github cache-strategy \
  --analyze-patterns \
...
npx @claude-flow/cli@latest github parallel-optimize \
  --analyze-dependencies \
...
npx @claude-flow/cli@latest github resource-pool \
  --share-agents \
  --distribute-load \
  --monitor-usage
```
...
npx @claude-flow/cli@latest github diagnose-connectivity \
  --test-all-repos \
  --check-permissions \
  --verify-webhooks
```
...
npx @claude-flow/cli@latest github debug-memory \
  --check-consistency \
  --identify-conflicts \
  --repair-state
```
...
npx @claude-flow/cli@latest github perf-analysis \
  --profile-operations \
...
npx @claude-flow/cli@latest github fullstack-update \
...
npx @claude-flow/cli@latest github cross-team \
